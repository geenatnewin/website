...
**Receipt-photo AI scan:** `app/api/ledger/analyze-receipt/route.js` uploads the photo to Cloudflare R2 (bucket `portfoliomedia`, under `receipts/`, **not publicly accessible** — unlike the site's videos, receipts are only ever served via a short-lived signed URL from `app/api/ledger/receipt/route.js`, generated by `app/dashboard/r2.js`) and sends it to Claude (`claude-sonnet-5` — upgraded from Haiku after it was misreading receipts, e.g. dumping full verbose product titles into item names and mislabeling tax as an item) with a JSON-schema structured output for reliable parsing. Returns `{ vendor, date, items[] }`, which the client uses to fill the form — always left for the user to review before submitting, never auto-submitted. Needs `ANTHROPIC_API_KEY`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_ACCOUNT_ID`, `R2_BUCKET_NAME` (all set in Vercel + `.env.local`).
