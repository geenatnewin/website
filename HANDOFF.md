...
**Receipt upload flow (Session 13):** picking photos no longer scans immediately — `ExpenseForm.js` holds them as `pendingFiles` (local blob previews only) with a Remove button per photo, and nothing is uploaded/sent to Claude until the **Scan N photos** button is clicked, so an accidental pick doesn't cost an API call. The file input takes `multiple`; all selected photos go in one scan request (`formData.getAll('receipt')` server-side) and are treated as one receipt (e.g. a long receipt shot in parts) — `analyze-receipt/route.js` sends every image in the same Claude message and uploads each to R2 individually, returning `receiptKeys[]`. The first key is stored in the existing `receipt_url` column for backward compatibility; the full array (when there's more than one) goes in `meta.receiptKeys`, read by both `ExpenseForm.js` (editing) and `ExpenseLine.js` (list view) via a shared fallback: prefer `meta.receiptKeys`, else wrap `receipt_url` in a single-item array, else empty.
