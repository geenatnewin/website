...
│   ├── icon.png                # Site favicon — tilted, softly-rounded organic oval with the vecteezy N-split-letter logo (auto-picked up by Next.js, no metadata wiring needed)
...
│   ├── dashboard/              # Password-protected gig income & tax expense tracker (see below) — URL is /dashboard, internal files still say "ledger" (ledger.css, LedgerShell.js, etc.). icon.png here is its own favicon — black "D" on the same oval shape as the site's icon.png, black-and-white instead of full color.
...
- **Favicon** — `app/icon.png`: the vecteezy N-split-letter logo (the road/swoosh-through-N mark) on a white, diagonally-tilted, subtly-organic oval (not a perfect ellipse — built from jittered points smoothed into a closed curve, corners rounded off just enough not to look sharp). Source is the 1920×1920 JPG on the Desktop (`vecteezy_n-split-letter-logo-...jpg`) — picked over the same-named `.eps` because the EPS's embedded preview was lower-res (1612×1612) and had TIFF corruption warnings when extracted. Generated with a throwaway `sharp`-based script (installed as a temp devDependency, uninstalled after) — the script itself isn't kept in the repo since it's a one-off; regenerate by re-deriving the same blob-path math if the logo ever needs to change again (see Session 14 log for the exact parameters: rotate -28°, aspect ratio ~1.35, 12-point jittered ellipse smoothed with a light single-pass moving average). `/dashboard`'s `app/dashboard/icon.png` reuses the identical oval shape, black-and-white, with a bold "D" instead of the logo image.
...
- `/dashboard` — Dashboard. Gradient stat cards (Income/Expenses/Mileage deduction/Net) with **click-to-reveal** amounts (blurred by default for privacy). "Gigs", "Expenses", and "Missing Payment" buttons expand a box containing that list *plus* its own "+ Add" button/form inside (`app/dashboard/DashboardSections.js`; form fields live in `GigForm.js` / `ExpenseForm.js`, reused for both add and edit). On successful add, the form clears and a green "ADDED" toast pops up bottom-center for ~1.8s. Every gig (`GigEntry.js`) and expense (`ExpenseLine.js`) has inline **Edit** and **Delete** controls — Edit swaps the row for the same form pre-filled, Delete asks for confirmation first (deleting a gig keeps its linked expenses, just unlinks them). **Missing Payment tab** (added Session 14): lists every gig with `status = 'pending'`, oldest-first, no month grouping (it's a follow-up list, not an archive) — a `$X across N gigs` total sits at the top, and the tab itself shows a yellow count badge (reuses the semantic yellow "pending" already means elsewhere, not a new accent color). No new server action — marking a gig paid is still done via its existing Edit form's Status dropdown. Empty state: "🎉 Nothing outstanding."
...
**Tailwind CSS — migration complete and deployed to navinng.com.** After both plain-CSS passes were rejected, the user asked directly for Tailwind. Installed `tailwindcss@3`/`postcss`/`autoprefixer`, scoped to `/dashboard` only (`tailwind.config.js` `content: ['./app/dashboard/**/*.{js,jsx}']`) so it can't touch the portfolio site's own hand-written CSS — `corePlugins.preflight: false` too, since `globals.css` (loaded on every route including `/dashboard`) already does its own reset and a second one from Tailwind would fight it. Verified this scoping actually holds by screenshotting the portfolio homepage after the migration — unaffected. Directives (`@tailwind base/components/utilities`) live at the top of `ledger.css` (already dashboard-scoped via its import in `layout.js`), so no new CSS entrypoint was needed. Theme extensions (`brand`/`ink` color scales) are in `tailwind.config.js`.
...
**Migration confirmed complete (Session 14 check, 2026-08-05):** the paragraph above once said the Gigs/Expenses lists, forms, modals, report, tips, security, and login pages were "not yet migrated" — that was stale, left over from mid-Session-13. Verified directly: `ledger.css` is down to only the deliberately-kept classes (chart SVG, `.ldg-censored`/`.ldg-revealed`, print markers, `.ldg-root` CSS vars), and grepping every `app/dashboard/**/*.js` file for `ldg-` turns up only references to those same kept classes — no component is still on old-system classNames. Every file in the old "not yet migrated" list is on Tailwind utilities against the `brand`/`ink` scale.
...
## Deploy Workflow (changed 2026-08-05)

Previously: auto-deploy after every change, no confirmation. **Now reversed** — for portfolio changes, run `npm run dev`, let the user preview at `localhost:3000`, and wait for explicit approval before `git add/commit/push` (which triggers Vercel's auto-deploy). After deploying, still re-sync the `navinnguyen.vercel.app` alias per the gotcha below.
...
- Gigs/Expenses lists grouped by month (see "Gigs/Expenses lists grouped by month" above), receipt reader gained discount/coupon handling and PDF support (see the corresponding sections above for both).
- **Full Tailwind CSS migration, deployed** (see "Tailwind CSS" and "Color direction settled" sections above for the whole story — two rejected plain-CSS passes, the explicit ask for Tailwind, landing on green/red/yellow, and every component's rebuild). Session ended here — this is the last entry as of 2026-07-30.
...
### Session 14 — 2026-08-05 to 2026-08-06 (New favicons, Missing Payment tab, deploy workflow reversed)
- **Corrected a stale HANDOFF claim:** the old "Not yet migrated" list (Gigs/Expenses lists, forms, modals, report, tips, security, login) said those were still on the old `.ldg-*` CSS system. Verified directly — they weren't. That was leftover text from mid-Session-13 that never got deleted once the migration actually finished. `ledger.css` only has the deliberately-kept classes (chart SVG, censor/reveal blur, print markers, root CSS vars); grepping every component for `ldg-` turns up nothing else.
- **Deploy workflow reversed:** was auto-deploy-after-every-change with no confirmation; user now wants `npm run dev` + localhost preview + explicit approval before pushing live. Applied for the rest of this session and saved to memory (`feedback-always-deploy.md`) so it persists across sessions. See "Deploy Workflow" section above.
- **New site favicon** (`app/icon.png`), iterated through several rounds of feedback:
  1. First pass: user's original N-monogram JPG (simple interlocking-N mark), trimmed and composited onto a plain white circle.
  2. User then provided the *actual* logo as an `.eps` — the vecteezy "N split-letter" mark (a road/swoosh cutting through the N) — plus a reference screenshot of a tilted oval favicon shape to match. No Ghostscript/Inkscape available, so extracted the EPS's own embedded preview image directly (DOS EPS binary format embeds a TIFF thumbnail — parsed the header's offset/length fields to pull it out) and used that as the source. Background changed from circle to a wider, diagonally-tilted oval (rotate -28°).
  3. User then supplied the source logo again as a plain 1920×1920 `.jpg` and asked which was better quality — the JPG won (higher-res, no TIFF corruption warnings the EPS preview had) and became the source going forward.
  4. "Bigger, and distort the shape while keeping it oval" — oval enlarged, edge given an organic wobble (12 points around the ellipse at jittered radii, connected with a smoothed Catmull-Rom curve instead of a perfect ellipse arc).
  5. "Round out the corners" — first pass rounded too much (lost the oval read entirely per user feedback: "now it's too round"), dialed back to a single light smoothing pass so only the sharpest points are softened.
  - Generated via a throwaway `sharp`-based script (installed as a temp devDependency each time, uninstalled after — not kept in the repo). Exact parameters as of the final version: `SIZE=256`, `ROTATE_DEG=-28`, oval aspect ratio ~1.35, 12-point jitter array `[1.0,1.08,0.94,1.06,0.92,1.07,0.95,1.05,0.93,1.06,0.95,1.03]` smoothed with one pass at `selfWeight=0.82`. If the logo needs regenerating again, rebuild this math rather than guessing from scratch.
- **New `/dashboard` favicon** (`app/dashboard/icon.png`), replacing the old purple→pink gradient "D" square (`icon.svg`, deleted): same oval shape/tilt/rounding as the main site's `icon.png` (identical blob-path math), but black-and-white — a bold black "D" instead of the logo image, on the same white oval. Reused instead of reinvented so the two favicons read as a matched pair. `middleware.js`'s auth-allowlist entry (the one that lets the favicon load before login) updated from `/dashboard/icon.svg` to `/dashboard/icon.png`.
- **New "Missing Payment" tab** on `/dashboard` (`DashboardSections.js`) — third tab next to Gigs/Expenses, listing every `status = 'pending'` gig oldest-first with a `$X across N gigs` total and a yellow count badge on the tab itself. Reuses the existing `GigEntry` row (Edit/Delete already worked); no new server action needed since marking paid is just editing the Status field. Verified visually via a throwaway Playwright script (temp devDependency, removed after) since curling the page alone can't show client-side tab-click state — logged in locally to screenshot both the closed and expanded states before/after the user's review.
- **Caught mid-session:** two local login attempts failed while testing — turned out to be my mistake (sent form-encoded data; `/api/ledger/login` expects JSON), not a wrong password, and cost 2 of the 3 attempts before lockout. Fixed by sending a proper JSON body. Worth remembering if testing the login API directly again.
- All changes this session went through the new localhost-preview-then-approve flow before deploying; each deploy was verified live on `navinng.com` afterward (favicon content-type/status checks, `/dashboard` still redirecting unauthenticated visitors) rather than assumed to have worked.
